...
Z wyrazami szacunku`,
  ...
  editor: {
    ...
    /** Panel „Dodaj z biblioteki" w terminie, etapach i cenniku (T-103). */
    docLibrary: {
      open: 'Dodaj z biblioteki',
      title: {
        schedule: 'Etapy terminu z biblioteki',
        stages: 'Etapy współpracy z biblioteki',
        price_list: 'Pozycje cennika z biblioteki',
      },
      hint: 'Kliknięcie dodaje od razu. Panel zostaje otwarty, ten sam wpis może wejść dwa razy.',
      search: 'Szukaj w bibliotece',
      count: (n: number) => `${n} w bibliotece`,
      addAll: (n: number) => `Dodaj wszystkie (${n})`,
      addLabel: (name: string) => `Dodaj z biblioteki: ${name}`,
      addedTimes: (n: number) => (n === 1 ? 'dodano' : `dodano ×${n}`),
...
Z wyrazami szacunku`,
  ...
  library: {
    ...
    /** Biblioteka dokumentow (T-102): termin, etapy wspolpracy, cennik. */
    docs: {
      tabs: {
        schedule: 'Termin',
        stages: 'Etapy współpracy',
      ...
      intro: {
        schedule:
          'Etapy, z których składa się szacowany termin. W dokumencie „Termin” dodajesz je z tej listy — albo wpisujesz własne.',
        stages:
          'Etapy współpracy, które opisujesz inwestorowi. W dokumencie zaznaczasz, które wchodzą w zakres.',
        price_list:
      ...
      namePlaceholder: {
        ...
        price_list: 'np. Panorama 360°',
      },
      emptyTitle: 'Ta sekcja biblioteki jest pusta',
      emptyDescription: 'Dodaj pierwszy wpis — będzie do wyboru przy tworzeniu dokumentu.',
      loadError: 'Nie udało się wczytać biblioteki dokumentów.',
      added: 'Dodano wpis',
      saved: 'Zapisano wpis',
      deleted: 'Usunięto wpis',
      deleteTitle: 'Usunąć wpis z biblioteki?',
      deleteDescription: (name: string) =>
        `„${name}” zniknie z biblioteki. Dokumenty, w których już jest, nie zmienią się.`,
      sampleBadge: 'Przykładowy',
      corrupted: 'Nie udało się odczytać tego wpisu.',
      summary: {
        days: 'dni',
      ...
      fields: {
        name: 'Nazwa',
        description: 'Opis',
        sectionLabel: 'Nagłówek grupy',
        sectionLabelHint: 'Pusty = pozycja bez grupy. Pozycje o tym samym nagłówku stoją razem.',
        owner: 'Kto zużywa czas',
        ownerProvider: 'Wykonawca (ARCH.)',
        ownerClient: 'Inwestor (INW.)',
        baseDays: 'Dni bazowe',
        roomScope: 'Pomieszczenia',
        roomScopeNone: 'Nie zależy od pomieszczeń',
        roomScopeVisual: 'Wizualne',
        roomScopeTechnical: 'Techniczne',
        roomScopeAll: 'Wszystkie',
        defaultPerRoomDays: 'Dni za pomieszczenie (domyślnie)',
        defaultPerRoomDaysHint:
          'Doliczane za każde pomieszczenie w zakresie etapu, pomnożone przez jego ilość (salon ×2 = dwa razy).',
        perRoomDays: 'Dni według typu pomieszczenia',
        perRoomDaysHint:
          'Puste pole = stawka domyślna. Zero to konkretna deklaracja: ten typ nie dokłada dni.',
        perRoomDaysNoTypes: 'Brak typów pomieszczeń — dodaj je w zakładce Pomieszczenia.',
        included: 'Domyślnie w zakresie',
        priceMin: 'Cena od',
        priceMax: 'Cena do',
        priceMaxHint: 'Puste = jedna cena, nie przedział.',
        unit: 'Jednostka',
        unitPlaceholder: 'np. h, m², szt.',
        leadTime: 'Termin realizacji',
        leadTimePlaceholder: 'np. 4–7 dni roboczych',
        addedDays: 'Dni do terminu',
        addedDaysHint: 'Ile dni roboczych dokłada do terminu. Puste = nie dotyczy.',
      },
    },
